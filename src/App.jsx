          ...
          if(isTfTemplate){
            ...
            if(tr.fileType==="ACT"){
              ...
              if(isGroup && uploadEntity && uploadEntity !== "consolidated") {
                setEntityActuals(prev => ({...prev, [uploadEntity]: {...(prev[uploadEntity]||{}), act: data}}));
                if(tr.actLast >= 0) setActLast(tr.actLast);
                setUploadMsg({text:"✓ ACT → "+((entities||[]).find(e=>e.id===uploadEntity)||{name:uploadEntity}).name+" — "+file.name,err:false});
              } else {
